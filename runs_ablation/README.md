# runs_ablation — Paper ablation archive

This folder packages the **finished ablation runs** used for the manuscript. Most panels lock training similarity at **0.3**; panel `04_similarity` additionally compares **0.3 / 0.5 / 0.7** at fixed `bal=10000`. Panel `05_seed_stability` re-trains the locked recipe at sim **0.3 / 0.7** across **4 seeds** to report 5-fold CV stability. Each experiment point is a **self-contained copy** of the original training output (including `checkpoints/`, `config.json`, and `results/`).

| Ablation | Path | #points |
|----------|------|--------:|
| Modality combination | `01_modality_ablation/` | 7 |
...
| Training-pair similarity threshold | `04_similarity/` | 3 |
...
| mod_s | 0.018 | 0.608 | 1.000 | 2.987 |

Full precision: `leaderboard.csv` / each run’s `results/summary.json`.

### Caveat (important for interpretation)

With `fusion_attn_mode=cross_qs`, **sequence-only (`mod_s`) is not a fair single-modality baseline**: Q comes from sequence while K/V are taken from disabled modalities (zeros), so the run collapses. Prefer multi-modality rows (`gsh/gs/sh/...`) for the main ablation narrative. A fairer single-modality comparison (concat fusion) lives outside this archive in `outputs_code_struct_s_concat_ablation/`.
...
python -m code.main train \
  ...
  --out-dir runs_ablation/01_modality_ablation/mod_gsh_repro \
  --name mod_gsh \
...
python -m code.main train \
  --config code/configs/gsh_struct_s_base.json \
  --out-dir runs_ablation/02_pair_balance_bin1/unsigned_bal10000_repro \
...
python -m code.main train \
  --config code/configs/gsh_struct_s_base.json \
  --out-dir runs_ablation/03_fusion_methods/attn_cross_qs_bal1000_repro \
...
Sweep: `similarity_threshold ∈ {0.3, 0.5, 0.7}` (minimum NW similarity for training pairs).

| point | sim | PCC | log10MAE | RSE | score |
|-------|----:|----:|---------:|----:|------:|
| sim0p3_bal10000 | 0.3 | **0.705** | **0.429** | **0.503** | **0.729** |
| sim0p5_bal10000 | 0.5 | 0.596 | 0.473 | 0.645 | 1.212 |
...
checkpoints/             # fold{1..5}_best.pt and fold{1..5}_last.pt  (10 files / run)
results/summary.json     # CV metrics (+ calibrator, etc.)
results/calibrator.json  # if present
intermediate/            # fold-wise z-score stats, logs
logs/                    # training logs (if present)
```

There are **29 runs** under `runs_ablation/` (including similarity ablation), each with `fold{1..5}_best.pt` + `fold{1..5}_last.pt`.

Verify a run:

```bash
ls runs_ablation/01_modality_ablation/mod_gsh/checkpoints/
python -c "import json; print(json.load(open('runs_ablation/01_modality_ablation/mod_gsh/results/summary.json'))['cv'])"
```
...
| Similarity threshold (0.3 / 0.5 / 0.7) | `04_similarity/sim0p*_bal10000` |
...
(same path via `runs_ablation/02_pair_balance_bin1/unsigned_bal10000/external_eval/`).
...
| `04_similarity/sim0p5_bal10000` | `outputs_code_struct_s_sim05_07_fusion/sim_bal_fusion__cross_qs_sim0p5_bal10000` |
| `04_similarity/sim0p7_bal10000` | `outputs_code_struct_s_sim05_07_fusion/sim_bal_fusion__cross_qs_sim0p7_bal10000` |

`MANIFEST.json` still records `source_path` (original `outputs_*` location) for provenance. The weights under `runs_ablation/**/checkpoints/` are independent copies and are sufficient for inference/evaluation without the original trees.
