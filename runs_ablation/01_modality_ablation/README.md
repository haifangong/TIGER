...
Each point directory is a full training run (`config.json`, `checkpoints/`,
`results/summary.json`, `intermediate/`, `logs/`).
...
With `cross_qs`, **`mod_s` is not a fair sequence-only baseline**: Query comes from
sequence while Key/Value come from disabled modalities (zeros), so the run
collapses. Prefer `gsh/gs/sh/...` for the main narrative. Fairer single-modality
baselines use concat fusion (see `03_fusion_methods/concat_gsh_bal10000` and
`outputs_code_struct_s_concat_ablation/`).
