...
python -m code.main train \
  ...
  --out-dir runs_ablation/06_seq_encoding/seq_encoding_grid__seq_onehot \
  --name seq_onehot \
...
- After completion, refresh `cv_leaderboard.json` with:
  `PYTHONPATH=. python scripts/run_struct_s_seq_encoding_grid.py --out-root runs_ablation/06_seq_encoding --leaderboard-only`
