"""Full classification metric suite for toxicity CV evaluation."""
...
def classification_metrics(
    ...
    y_score = yp.astype(float) if y_prob is None else np.asarray(y_prob, dtype=float).ravel()
    return {
        "n": int(len(yt)),
        "n_pos": int((yt == 1).sum()),
        "n_neg": int((yt == 0).sum()),
        "accuracy": float(accuracy_score(yt, yp)) if len(yt) else float("nan"),
        "precision": float(precision_score(yt, yp, average=average, zero_division=zero_division)),
...
def summarize_cv(fold_rows: list[dict]) -> pd.DataFrame:
    ...
    for model, g in df.groupby("model"):
        row_mean = {"split": "cv_mean", "model": model, "fold": "mean"}
        row_std = {"split": "cv_std", "model": model, "fold": "std"}
        for col in METRIC_COLUMNS:
            row_mean[col] = float(g[col].mean())
            row_std[col] = float(g[col].std(ddof=0))
        row_mean["n"] = int(g["n"].sum())
