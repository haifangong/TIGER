#!/usr/bin/env bash
# Launch all MIC paper ablation panels (01–05) under runs_ablation_repro/ (05 → archive).
# Panels run **sequentially** (each blocks until its suite finishes) so they do not
# contend for the same GPUs. Default seed for 01–04 is 1; panel 05 sweeps 1–5.
#
# Usage (from TIGER/):
...
echo "[ablation] launching MIC panels 01→05 sequentially (FOREGROUND=1)"
